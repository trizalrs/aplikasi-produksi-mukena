...
import { TrashIcon, CameraIcon, ChevronDownIcon, DownloadIcon } from './Icons';
import jsPDF from 'jspdf';
import autoTable from 'jspdf-autotable';

// --- Helper untuk export PDF (tidak berubah) ---
...
// --- Sub-komponen Laporan Riwayat Gajian ---
function LaporanRiwayatGajian({ riwayatGajian, formatCurrency, formatDate, handleDelete, handlePrintRequest, pegawai }) {
    ...
    return ( 
        <div className="space-y-6"> 
            <div className="bg-white p-4 rounded-xl shadow-lg no-print"> 
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end"> 
                    <div> <label className="block text-sm font-medium text-gray-700">Pegawai</label> <select name="pegawaiId" value={filters.pegawaiId} onChange={handleFilterChange} className="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"> <option value="semua">Semua Pegawai</option> {pegawai.map(p => <option key={p.id} value={p.id}>{p.nama}</option>)} </select> </div> 
                    <div> <label className="block text-sm font-medium text-gray-700">Dari Tanggal</label> <input type="date" name="startDate" value={filters.startDate} onChange={handleFilterChange} className="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"/> </div> 
                    <div> <label className="block text-sm font-medium text-gray-700">Sampai Tanggal</label> <input type="date" name="endDate" value={filters.endDate} onChange={handleFilterChange} className="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"/> </div> 
                </div> 
            </div> 
                            ...
                            {filteredRiwayat.map(item => ( 
                                <tr key={item.id}> 
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-center text-sm font-medium space-x-2"> 
                                        {/* --- PERUBAHAN DI SINI --- */}
                                        <button onClick={() => handlePrintRequest('slip', item)} className="text-blue-600 hover:text-blue-900" title="Cetak Slip"> <CameraIcon /> </button> 
                                        <button onClick={() => handleDelete(item.id)} className="text-red-600 hover:text-red-900" title="Hapus Riwayat"> <TrashIcon /> </button> 
                                    </td> 
                                </tr> 
                            ))} 
                        </tbody> 
            ...
            ) : ( <div className="text-center bg-white p-10 rounded-lg shadow"><h3 className="text-lg font-semibold text-gray-700">Tidak ada riwayat gajian yang cocok dengan filter.</h3></div> )} 
        </div> 
    );
}

// --- Sub-komponen Laporan Riwayat Kasbon ---
function LaporanRiwayatKasbon({ kasbon, formatCurrency, formatDate, handlePrintRequest, pegawai }) {
    const [filters, setFilters] = useState({ startDate: '', endDate: '', pegawaiId: 'semua' });
    const handleFilterChange = (e) => { const { name, value } = e.target; setFilters(prev => ({ ...prev, [name]: value })); };
    ...
    return ( 
        <div className="space-y-6"> 
            ...
            {filteredRiwayat.length > 0 ? ( 
                <div className="bg-white rounded-xl shadow-lg overflow-x-auto"> 
                    <table className="min-w-full divide-y divide-gray-200"> 
                        ...
                        <tbody className="bg-white divide-y divide-gray-200"> 
                            {filteredRiwayat.map(item => ( 
                                <tr key={item.id} className={item.status === 'dibatalkan' ? 'bg-gray-100 text-gray-400' : ''}> 
                                    <td className="px-6 py-4 whitespace-nowrap text-sm">{formatDate(item.tanggal)}</td> 
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">{item.pegawaiNama}</td> 
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-red-600">{formatCurrency(item.jumlah)}</td> 
                                    <td className="px-6 py-4 whitespace-nowrap text-sm"> <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${item.status === 'dibatalkan' ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'}`}> {item.status || 'aktif'} </span> </td> 
                                    <td className="px-6 py-4 whitespace-nowrap text-center text-sm font-medium"> 
                                        {/* --- PERUBAHAN DI SINI --- */}
                                        <button onClick={() => handlePrintRequest('kasbon', item)} className="text-blue-600 hover:text-blue-900" title="Cetak Ulang Struk"> <CameraIcon /> </button> 
                                    </td> 
                                </tr> 
                            ))} 
                        </tbody> 
            ...
            ) : (<div className="text-center bg-white p-10 rounded-lg shadow"><h3 className="text-lg font-semibold text-gray-700">Tidak ada riwayat kasbon yang cocok dengan filter.</h3></div>)} 
        </div> 
    );
}

// --- Sub-komponen Laporan Produksi (tidak berubah) ---
...
function LaporanDataPegawai({ pegawai }) { /* ... (komponen ini tidak berubah) ... */ }
function LaporanDataProduk({ produk, formatCurrency }) { /* ... (komponen ini tidak berubah) ... */ }

// --- Komponen Utama Halaman Laporan ---
function PageLaporan({ riwayatGajian, kasbon, formatCurrency, formatDate, handleRiwayatGajianDelete, transaksi, pegawai, produk, showNotification, handlePrintRequest }) {
    ...
    return (
        <div>
            ...
            <div>
                {activeReport === 'riwayatGajian' && <LaporanRiwayatGajian riwayatGajian={riwayatGajian} formatCurrency={formatCurrency} formatDate={formatDate} handleDelete={handleRiwayatGajianDelete} handlePrintRequest={handlePrintRequest} pegawai={pegawai} />}
                {activeReport === 'riwayatKasbon' && <LaporanRiwayatKasbon kasbon={kasbon} formatCurrency={formatCurrency} formatDate={formatDate} handlePrintRequest={handlePrintRequest} pegawai={pegawai} />}
