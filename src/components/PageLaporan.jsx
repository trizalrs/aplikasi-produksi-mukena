...
const exportToPdf = (title, head, body, fileName) => {
    ...
    if(periode) { doc.setFontSize(10); doc.text(periode, 14, 28); }
    autoTable(doc, { startY: 35, head: head, body: body, theme: 'striped', headStyles: { fillColor: [41, 128, 185] }, didParseCell: function (data) { if (data.cell.raw?.isGroupHeader) { data.cell.styles.fillColor = '#eaf2f8'; data.cell.styles.fontStyle = 'bold'; } } });
    doc.save(fileName);
};
    ...
    const handleExport = () => {
        ...
        if (filters.groupBy === 'pegawai') {
            reportData.byEmployee.forEach(emp => {
                body.push([{ content: emp.name, raw: {isGroupHeader: true} }, { content: `${emp.totalPcs} Pcs`, raw: {isGroupHeader: true}, styles: { halign: 'right' } }, { content: formatCurrency(emp.totalUpah), raw: {isGroupHeader: true}, styles: { halign: 'right' } }]);
            ...
            reportData.byProduct.forEach(prod => {
                body.push([{ content: prod.name, raw: {isGroupHeader: true} }, { content: `${prod.totalPcs} Pcs`, raw: {isGroupHeader: true}, styles: { halign: 'right' } }, { content: formatCurrency(prod.totalUpah), raw: {isGroupHeader: true}, styles: { halign: 'right' } }]);
                prod.variants.forEach(v => { body.push([`  ${v.name}`, { content: `${v.totalPcs} Pcs`, styles: { halign: 'right' } }, { content: formatCurrency(v.totalUpah), styles: { halign: 'right' } }]); });
            });
        }
