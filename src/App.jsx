...
function App() {
    ...
    const restoreAllData = (restoredData) => {
        if (restoredData && Array.isArray(restoredData.pegawai) && Array.isArray(restoredData.produk)) {
            setPegawai(restoredData.pegawai || []);
            setProduk(restoredData.produk || []);
            setTransaksi(restoredData.transaksi || []);
            setKasbon(restoredData.kasbon || []);
            setPembayaranKasbon(restoredData.pembayaranKasbon || []);
            setRiwayatGajian(restoredData.riwayatGajian || []);
            if (restoredData.authData) {
                localStorage.setItem('authData', JSON.stringify(restoredData.authData));
                setAuthData(restoredData.authData);
            }
    ...
    const handleAppLogout = () => {
        ...
        handleKonfirmasi('Logout', message, () => {
            resetKonfirmasi();
            if (isDriveConnected) {
                // Panggil fungsi backup dari ref, dan tunggu hasilnya
                driveRef.current.backup()
                    .then(() => {
                        // Jika backup berhasil, tampilkan notif lalu logout
                        showNotification('Backup berhasil! Anda akan logout...', 'success');
                        setTimeout(() => logout(), 2000);
                    })
                    .catch(err => {
                        // Jika backup gagal, tampilkan notif dan JANGAN logout
            ...
            <div className="main-app no-print">
                ...
                <header className="bg-white shadow-md p-4 sticky top-0 z-40">
                    <div className="container mx-auto flex flex-col md:flex-row md:justify-between md:items-center gap-4">
                        ...
                        <div className="flex items-center space-x-2 flex-wrap gap-2 justify-start w-full md:w-auto">
                            <GoogleDriveSync
                                ref={driveRef}
                            ...
                            <div className="flex items-center space-x-2">
                                <label className="text-sm font-medium text-gray-600 flex-shrink-0">Ukuran Struk:</label>
                            ...
                            <button onClick={handleBackupLokal} className="flex items-center bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-3 rounded-lg text-sm"> <DownloadIcon /> Backup Lokal </button>
                            <button onClick={() => fileInputRef.current.click()} className="flex items-center bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-3 rounded-lg text-sm"> <UploadIcon /> Restore Lokal </button>
                            <input type="file" ref={fileInputRef} onChange={handleRestoreLokal} accept=".json" className="hidden" />
                            <button onClick={handleAppLogout} className="flex items-center bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-3 rounded-lg text-sm"> Logout </button>
                        </div>
                    </div>
                </header>
